...
* The file [srapcontributors.csv](srapcontributors.csv) attempts to follow the [diagram](https://github.com/dcmi/dc-srap/issues/94#issuecomment-3878558125). The relevant shapes are Person, Organization, and Contribution. The flow is from SRAPResource to Contribution; then a dct:contributor property in the Contribution shape has a valueShape of either Person or Organization. The TAP also has dct:creator and dct:contributor that link to Person and Organization. These rows will need to be removed if we decide that the model is to be:
```
